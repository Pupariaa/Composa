...
async function run() {
    ...
    const { html, subject } = testMailer.compileMail('password-reset', {
        lang: 'fr',
        variables: { USER_NAME: 'Utilisateur Test', USER_EMAIL: 'test@example.com', RESET_URL: 'https://yourapp.com/reset/test123', EXPIRATION_TIME: '1 heure' }
    });

    const result = await testMailer.sendMail({ to: 'test@example.com', html, subject });
